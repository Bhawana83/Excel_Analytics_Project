...
const AdminUserDetails = () => {
  ...
  const fetchUserDetails = async () => {
    ...
    try {
      setLoading(true);
      const res = await axiosInstance.get(API_PATHS.ADMIN.GET_USER_DETAILS(id));
  ...
  const handleDownload = async (fileId, fileName) => {
    try {
      const res = await axiosInstance.get(
        API_PATHS.ADMIN_FILE.DOWNLOAD_FILE(fileId),
        { responseType: "blob" }
      );
  ...
  const handleDelete = async (fileId) => {
    try {
      await axiosInstance.delete(API_PATHS.ADMIN_FILE.DELETE_FILE(fileId));
  ...
  if (error) {
    return (
      <DashboardLayout activeMenu="Get User">
