...
// const TOTAL_SECONDS = 300; // 5 minutes
// // const TOTAL_SECONDS = 60; // 5 minutes

// export default function AdminPending() {
//   //   const location = useLocation();
//   const navigate = useNavigate();

//   // Use Context API to handle user registration
//   const { updateUser } = useContext(UserContext);

//   // 🟢 CHANGE: get userId from route state OR localStorage (survives reload)
//   const [pendingId] = useState(
//     () => localStorage.getItem("pendingAdminId") || ""
//   );
...
//   // 🟢 CHANGE: progress for conic ring
//   const progress = useMemo(
//     () => ((TOTAL_SECONDS - timeLeft) / TOTAL_SECONDS) * 100,
//     [timeLeft]
//   );
...
//     // poll backend every 7s for status
//     const poll = setInterval(async () => {
//       try {
//         console.log(pendingId)
//         const { data } = await axiosInstance.get(API_PATHS.ADMIN.STATUS(pendingId));
...
//           try {
//             // fetch token now that admin is approved
//             const res = await axiosInstance.post(
//               API_PATHS.AUTH.APPROVED_LOGIN(pendingId)
//             );
//             const { token, user } = res.data;
...
//             navigate("/dashboard");
//           } catch (err) {
//             toast.error("Unable to login automatically after approval.");
//             console.error(err);
//           }
//         }
...
//       clearInterval(poll);
//     };
//   }, [pendingId, navigate]); // ✅ only depends on userId and navigate

//   return (
//     <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-indigo-50 to-white px-4">
//       <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl border p-6 sm:p-8">
//         <div className="flex items-center gap-3 mb-4">
//           <span className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 text-xl">
//             ⏳
//           </span>
...
//           Keep this page open. We’ll auto-redirect you once your admin request
//           is approved.
//         </p>

//         {/* 🟢 CHANGE: animated countdown card */}
//         <div className="mt-6 flex items-center justify-center">
//           <motion.div
//             initial={{ scale: 0.9, opacity: 0 }}
//             animate={{ scale: 1, opacity: 1 }}
//             transition={{ type: "spring", stiffness: 200, damping: 20 }}
//             className="relative h-44 w-44 rounded-full grid place-items-center"
//             style={{
//               background: `conic-gradient(#4f46e5 ${progress}%, #e5e7eb 0)`,
//             }}
//           >
//             <div className="absolute inset-3 bg-white rounded-full shadow-inner grid place-items-center">
//               <AnimatePresence mode="wait">
//                 <motion.div
//                   key={mmss}
//                   initial={{ y: 10, opacity: 0 }}
...
//                   exit={{ y: -10, opacity: 0 }}
//                   transition={{ duration: 0.2 }}
//                   className="text-3xl sm:text-4xl font-mono font-bold text-gray-800"
//                 >
//                   {mmss}
//                 </motion.div>
//               </AnimatePresence>
...
//           <button
//             onClick={() => navigate("/")}
//             className="w-full py-2 rounded-lg border hover:bg-gray-50 transition"
//           >
//             Go Home
//           </button>
//           <button
...
//                   API_PATHS.ADMIN.STATUS(pendingId)
//                 );
//                 toast(`Current status: ${data?.status || "unknown"}`);
//               } catch (error) {
//                 toast.error("Unable to fetch status : ", error.message);
//               }
//             }}
